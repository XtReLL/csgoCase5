...
export class PromocodeService {
  ...
  async removePromocode(    
    ...
    const promocode = await this.promocodeRepository.findOneOrFail(parseInt(id, 10))
    await this.promocodeRepository.softRemove(promocode)
    return true;
	}

	async usePromocode(user: User, code: string): Promise<any> {
		if (typeof await this.redisClientService.get(`use_promo_${user.id}`) !== 'undefined') {
			throw new HttpException('There are too many requests, please wait', 400)
		}

		this.redisClientService.set(`use_promo_${user.id}`, code, {ttl: 1000})

		if (!await this.userService.getProfileVisibleSteam(user.steamId)) {
			throw new Error("To use the promo code, you must have a public STEAM profile")
		}

		if (await this.userService.getSteamLevel(user.steamId) < await this.redisClientService.get('min_steam_lvl_for_use_promocode')) {
			throw new Error("To use the promo code, you must have STEAM level 2 or higher")
		}

		if (await this.userService.getPlayTimeCSGO(user.steamId) < await this.redisClientService.get('min_play_time_in_csgo_for_use_promocode')) {
			throw new Error("To use the promo code, you must have at least 30 hours in CS: GO")
		}
		...
		if (await this.promocodeUseRepository.findOne({
				where: {
